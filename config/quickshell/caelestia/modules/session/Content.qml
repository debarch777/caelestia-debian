...
Column {
    ...
    component SessionButton: IconButton {
        ...
        function exec(): void {
            if (command.length === 1 && command[0] === "hyprland_exit") {
                Hypr.exit();
                Quickshell.execDetached(["sh", "-c", "hyprctl dispatch exit || hyprctl 'eval hl.dispatch(hl.dsp.exit())'"]);
                return;
            }
