...
Item {
    ...
    readonly property string msg: {
        // Errors
        if (pam.fprint.available && pam.fprint.state === Pam.Error)
            return qsTr("FP ERROR: %1").arg(pam.fprint.message);
        if (pam.howdy.available && pam.howdy.state === Pam.Error)
        ...
        if (pam.state !== Pam.MaxTries) {
            if (pam.fprint.available && pam.fprint.state === Pam.Failed)
            ...
            if (pam.howdy.available && pam.howdy.state === Pam.Failed)
    ...
    readonly property string stateMsg: {
        if (Hypr.kbLayout !== "??" && Hypr.defaultKbLayout !== "??" && Hypr.kbLayout !== Hypr.defaultKbLayout) {
