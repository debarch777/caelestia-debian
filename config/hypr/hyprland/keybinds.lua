...
create_bind(vars.kbLogoutDirect, hl.dsp.exit())
...
-- Workspaces
for i = 0, 9 do
    create_bind(extend_keybind(vars.kbGoToWs, tostring(i)), hl.dsp.focus({ workspace = tostring(i) }))
    create_bind(extend_keybind(vars.kbGoToWsGroup, tostring(i)), hl.dsp.focus({ workspace = tostring(i * 10) }))
    create_bind(extend_keybind(vars.kbMoveWinToWs, tostring(i)), hl.dsp.window.move({ workspace = tostring(i) }))
    create_bind(extend_keybind(vars.kbMoveWinToWsGroup, tostring(i)), hl.dsp.window.move({ workspace = tostring(i * 10) }))
end
...
-- Resize window
create_bind(vars.kbWindowDecreaseWidth, hl.dsp.window.resize({ width = -20 }), repeating)
create_bind(vars.kbWindowIncreaseWidth, hl.dsp.window.resize({ width = 20 }), repeating)
create_bind(vars.kbWindowDecreaseHeight, hl.dsp.window.resize({ height = -20 }), repeating)
create_bind(vars.kbWindowIncreaseHeight, hl.dsp.window.resize({ height = 20 }), repeating)
...
create_bind(vars.kbWindowPip, function()
    ...
    if a then
        local pip = fn.move_actions(a.title, a.class) or {}
...
create_bind(vars.kbWhatsapp, hl.dsp.exec_cmd("whatsapp"))
create_bind(vars.kbHandySTT, hl.dsp.exec_cmd("pkill -SIGUSR2 -x handy || handy --start-hidden"))
create_bind(vars.kbHandySTTPostProcess, hl.dsp.exec_cmd("pkill -SIGUSR1 -x handy || handy --start-hidden"))

-- Wallpapers
create_bind(vars.kbWallpaperRandom, hl.dsp.exec_cmd("caelestia wallpaper -r " .. os.getenv("HOME") .. "/Pictures/Wallpapers/"))
create_bind(vars.kbWallpaperSelect, hl.dsp.global("caelestia:nexus"))
