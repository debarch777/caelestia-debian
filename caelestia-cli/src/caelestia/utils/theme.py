...
def apply_discord(scss: str) -> None:
    ...
    for client in "Equicord", "Vencord", "BetterDiscord", "equibop", "vesktop", "legcord":
        ...
        atomic_write(Path.home() / f".var/app/dev.vencord.Vesktop/config/{client}/themes/caelestia.theme.css", conf)
        atomic_write(Path.home() / f".var/app/com.discordapp.Discord/config/{client}/themes/caelestia.theme.css", conf)
...
def fast_papirus_recolor(color: str, theme: str = "Papirus-Dark") -> None:
    icon_base = Path.home() / f".local/share/icons/{theme}"
    if not icon_base.is_dir():
        return

    pattern = re.compile(r'^(folder|user|custom)-(' + '|'.join(KNOWN_PAPIRUS_COLORS) + r')(.*\.svg)$')

    for root_dir, dirs, files in os.walk(icon_base):
        if not ("places" in root_dir or "actions" in root_dir):
            continue
        for f in files:
            ...
            if os.path.islink(full_path):
                ...
                if match:
                    ...
                    new_target = f"{prefix}-{color}{suffix}"
                    target_file = os.path.join(root_dir, new_target)
                    if os.path.exists(target_file):
                        try:
                            ...
                            pass
                elif target.startswith("folder_") or target.startswith("folder-"):
                    for old_c in KNOWN_PAPIRUS_COLORS:
                        if old_c in target:
                            new_target = target.replace(old_c, color)
                            if os.path.exists(os.path.join(root_dir, new_target)):
                                try:
                                    os.remove(full_path)
                                    os.symlink(new_target, full_path)
                                except Exception:
...
def sync_papirus_colors(hex_color: str, mode: str = "dark") -> None:
    """Ultra-fast synchronous folder icon tinting across all categories"""
    try:
        ...
        if saturation < 8:
            ...
            color = _determine_hue_color(r, g, b, brightness, True)
        else:
            color = _determine_hue_color(r, g, b, brightness, False)

        for theme in ["Papirus", "Papirus-Dark", "Papirus-Light"]:
            fast_papirus_recolor(color, theme)
            p = Path.home() / f".local/share/icons/{theme}"
            if p.exists():
                p.touch()

        # Invalidate GTK in-memory icon cache
        current_theme = "Papirus" if mode == "light" else "Papirus-Dark"
        temp_theme = "Papirus-Dark" if mode == "light" else "Papirus"
        subprocess.run(["gsettings", "set", "org.gnome.desktop.interface", "icon-theme", temp_theme], stderr=subprocess.DEVNULL, check=False)
        subprocess.run(["gsettings", "set", "org.gnome.desktop.interface", "icon-theme", current_theme], stderr=subprocess.DEVNULL, check=False)
    except Exception:
        pass
...
def apply_gtk(colours: dict[str, str], mode: str, icon_theme: str | None = None) -> None:
    ...
    subprocess.run(["dconf", "write", "/org/gnome/desktop/interface/gtk-theme", f"'{gtk_theme}'"], stderr=subprocess.DEVNULL, check=False)
    subprocess.run(["gsettings", "set", "org.gnome.desktop.interface", "gtk-theme", f"'{gtk_theme}'"], stderr=subprocess.DEVNULL, check=False)

    subprocess.run(["dconf", "write", "/org/gnome/desktop/interface/color-scheme", f"'prefer-{mode}'"], stderr=subprocess.DEVNULL, check=False)
    subprocess.run(["gsettings", "set", "org.gnome.desktop.interface", "color-scheme", f"'prefer-{mode}'"], stderr=subprocess.DEVNULL, check=False)

    gtk_icon_theme = icon_theme if icon_theme is not None else ("Papirus" if mode == "light" else "Papirus-Dark")
    subprocess.run(["dconf", "write", "/org/gnome/desktop/interface/icon-theme", f"'{gtk_icon_theme}'"], stderr=subprocess.DEVNULL, check=False)
    subprocess.run(["gsettings", "set", "org.gnome.desktop.interface", "icon-theme", f"'{gtk_icon_theme}'"], stderr=subprocess.DEVNULL, check=False)

    sync_papirus_colors(colours["primary"], mode)
    subprocess.run(["thunar", "-q"], stderr=subprocess.DEVNULL, check=False)
    subprocess.run(["pkill", "-x", "thunar"], stderr=subprocess.DEVNULL, check=False)
    time.sleep(0.05)
    subprocess.Popen(["thunar", "--daemon"], stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL)
...
def apply_chromium(colours: dict[str, str]) -> None:
    # Do not call --refresh-platform-policy as it freezes active browser windows
    pass
